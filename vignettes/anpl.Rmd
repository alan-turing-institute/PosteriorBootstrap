...
```{r echo = T, results = 'hide'}
train_dat <- list(n = n_input, p = p, x = x, y = y, beta_sd = prior_sd)
...
bayes_sample <- rstan::extract(bayes_log_reg)$beta
```
...
for (concentration in concentrations) {
  anpl_sample <- PosteriorBootstrap::anpl(x = german$x, y = german$y,
                                          concentration = concentration,
...
Our construction admits a trivially parallelizable sampler once the parametric
...
We now illustrate the paralellisation speedup. The calculation of the expected
...
duration of the task with `N` processors. Due to limitations in the R workflow,
[we are limited to one or two
...
but see the [GitHub
README](https://github.com/alan-turing-institute/PosteriorBootstrap/blob/master/README.md)
for a plot with 8 cores.  We compute the duration for one or two cores and for
the number of bootstrap samples in 100, 1000, and 10,000.
...
for (n_bootstrap in n_bootstrap_array) {
  ...
  for (num_cores in seq(1, max_cores)) {
    ...
    start <- Sys.time()
    anpl_samples <- PosteriorBootstrap::anpl(x = german$x, y = german$y,
                                             concentration = 1,
                                             n_bootstrap = n_bootstrap,
...
ggplot(data = speedups,
       aes(x = Num_cores, y = speedup)) + geom_line(aes(colour = N_bootstrap))
```
...
qplot(proportion, data = speedups, fill = N_bootstrap, binwidth = 0.005) +
  facet_wrap(facets = ~N_bootstrap)
```
