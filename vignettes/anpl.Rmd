...
We require our package and the `rstan` package for variational Bayes:
...
n_bootstrap <- 1000
set.seed(1)
german <- PosteriorBootstrap::load_dataset(list(name = "statlog-german-credit.dat"))
```
...
stan_vb_sample <- PosteriorBootstrap::run_variational_bayes(x = cbind(1, dataset$x),
                                                            y = 0.5 * (dataset$y + 1),
															output_samples = 10,
															beta_sd = sqrt(prior_variance),
															iter = 100)
print(str(stan_vb_sample))
```
